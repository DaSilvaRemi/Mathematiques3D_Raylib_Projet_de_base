...
void MyDrawSpherePortion(Quaternion q, Sphere sph, float startTheta, float endTheta, float startPhi, float endPhi, int nSegmentsTheta, int nSegmentsPhi, Color color)
{
	...
	for (int i = 0; i < nSegmentsTheta + 1; i++)
		vertexBufferTheta[i] = SphericalToCartesian({ 1, Lerp(startTheta, endTheta, i / static_cast<float>(nSegmentsTheta)), startPhi });
...
void MyDrawRoundedBoxV2(RoundedBox roundedBox, Color color)
{
	...
	Referential refQuadBack = Referential(Vector3Add(posRef, { roundedBox.extension.x / 2, roundedBox.extension.y / 2, -(roundedBox.extension.z + roundedBox.radius) }), qBackQuad);
	...
	Referential refQuadRight = Referential(Vector3Add(posRef, { roundedBox.extension.x + roundedBox.radius,roundedBox.extension.y / 2, -roundedBox.extension.z / 2 }), qRightQuad);
	...
	Referential refQuadLeft = Referential(Vector3Add(posRef, { -roundedBox.radius, roundedBox.extension.y / 2, -roundedBox.extension.z / 2 }), qLeftQuad);
	...
	Referential refQuadTop = Referential(Vector3Add(posRef, { roundedBox.extension.x / 2,roundedBox.extension.y + roundedBox.radius,-roundedBox.extension.z / 2 }), qTopQuad);
	...
	Referential refQuadBottom = Referential(Vector3Add(posRef, { roundedBox.extension.x / 2, -roundedBox.radius,-roundedBox.extension.z / 2 }), qBottomQuad);
