...
Vector3 GlobalToLocalPos(Vector3 posGlobal, Referential localRef)
{
    ...
    Vector3 globalVect = Vector3Subtract(Vector3Subtract(posGlobal, globalOrigin),
                                         Vector3Subtract(localRef.origin, globalOrigin));
...
{
    return {
        Vector3DotProduct(vectGlobal, localRef.i), Vector3DotProduct(vectGlobal, localRef.j),
...
{
    return Vector3Add(Vector3Add(Vector3Scale(localRef.i, localVect.x), Vector3Scale(localRef.j, localVect.y)),
...
{
    bool isIntersec = InterSegPlane(
        seg, Plane(GlobalToLocalPos(disk.referential.i, disk.referential), disk.referential.origin), interPt,
        interNormal);
    if (!isIntersec) return false;
...
bool InterSegmentInfiniteCylinder(Segment seg, Cylinder cyl, Vector3& interPt, Vector3& interNormal)
{
    ...
    float b = 2 * (ABdotPA - ABPQsurPQcarre * PAdotPQ - PAPQsurPQcarre * ABdotPQ + ABPQsurPQcarre * PAPQsurPQcarre *
        PQcarre);
...
bool InterSegmentCapsule(Segment seg, Capsule caps, Vector3& interPt, Vector3& interNormal)
{
    ...
    auto ref = Referential(down);
    ref.RotateByQuaternion(caps.referential.q);
    auto cylinder = Cylinder(ref, caps.radius, caps.height);
...
bool IntersecSegRoundedBox(Segment seg, RoundedBox roundedBox, Vector3& interPt, Vector3& interNormal)
{
    ...
    Quad quadFront = {
        Referential(Vector3Add(posRef, {roundedBox.extension.x / 2, roundedBox.extension.y / 2, roundedBox.radius}),
                    qFrontQuad),
        {roundedBox.extension.x, roundedBox.extension.z, roundedBox.extension.y}
    };
    ...
    Quad quadRight = {
        Referential(Vector3Add(posRef, {
                                   roundedBox.extension.x + roundedBox.radius, roundedBox.extension.y / 2,
                                   -roundedBox.extension.z / 2
                               }), qRightQuad),
        {roundedBox.extension.y, roundedBox.extension.x, roundedBox.extension.z}
    };
    ...
    Quad quadLeft = {
        Referential(Vector3Add(posRef, {-roundedBox.radius, roundedBox.extension.y / 2, -roundedBox.extension.z / 2}),
                    qLeftQuad),
        {roundedBox.extension.y, roundedBox.extension.x, roundedBox.extension.z}
    };
    ...
    Quad quadTop = {
        Referential(Vector3Add(posRef, {
                                   roundedBox.extension.x / 2, roundedBox.extension.y + roundedBox.radius,
                                   -roundedBox.extension.z / 2
                               }), qTopQuad),
        {roundedBox.extension.x, roundedBox.extension.y, roundedBox.extension.z}
    };
    ...
    Quad quadBottom = {
        Referential(Vector3Add(posRef, {roundedBox.extension.x / 2, -roundedBox.radius, -roundedBox.extension.z / 2}),
                    qBottomQuad),
        {roundedBox.extension.x, roundedBox.extension.y, roundedBox.extension.z}
    };
...
bool IntersecSegBox(Segment seg, Box box, Vector3& interPt, Vector3& interNormal)
{
    ...
    auto referentialQuadTop = Referential(Vector3Add(box.ref.origin, {0, box.extension.y / 2, 0}), qTop);
    auto referentialQuadFront = Referential(Vector3Add(box.ref.origin, {box.extension.x / 2, 0, 0}), qFront);
    auto referentialQuadBack = Referential(Vector3Add(box.ref.origin, {-box.extension.x / 2, 0, 0}), qBack);
    auto referentialQuadLeft = Referential(Vector3Add(box.ref.origin, {0, 0, box.extension.z / 2}), qLeft);
    auto referentialQuadRight = Referential(Vector3Add(box.ref.origin, {0, 0, -box.extension.z / 2}), qRight);
    auto referentialQuadBottom = Referential(Vector3Add(box.ref.origin, {0, -box.extension.y / 2, 0}), qBottom);
